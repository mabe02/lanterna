...
/**
 * Represents a key pressed. Use getKind() to see if it's a normal alpha-numeric key or any special key. Sorry if the
 * special keys are sort of european-language centered, that's unfortunately the only keyboard I have to test this with.
 *
 * @author Martin
 */
public class Key {
    ...
    /**
     * What kind of key was pressed? This is a bit misleading as we consider 'normal' keys one kind but then have one
     * kind per special keys. So, "Page Up" is one kind, "Insert" is one kind but "G" is a normal key. If getKind()
     * returns Kind.NormalKey, you can use getCharacter() to see exactly which character was typed in.
     */
    public Kind getKind() {
        return kind;
    }
